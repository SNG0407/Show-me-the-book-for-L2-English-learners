...
public class GrabIt : MonoBehaviour {
	...
	void Rotate()
	{
		if(Input.GetKeyDown(m_rotatePitchPosKey)){
				//m_targetRB.AddTorque(  m_transform.right * m_angularSpeed );		
				m_targetRB.transform.Rotate(Vector3.right, 90f);	
			}
			else if(Input.GetKeyDown(m_rotatePitchNegKey)){
				m_targetRB.transform.Rotate(Vector3.right, -90f);
			//m_targetRB.AddTorque(  - m_transform.right * m_angularSpeed );
		}

		if(Input.GetKeyDown(m_rotateYawPosKey)){
				m_targetRB.transform.Rotate(Vector3.up, 90f);
				//m_targetRB.AddTorque( - m_transform.up * m_angularSpeed );
			}
			else if(Input.GetKeyDown(m_rotateYawNegKey)){
				m_targetRB.transform.Rotate(Vector3.up, -90f);
			//m_targetRB.AddTorque( m_transform.up * m_angularSpeed );
		}
	}
